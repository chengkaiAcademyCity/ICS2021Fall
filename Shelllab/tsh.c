...
volatile sig_atomic_t flag; /* fg pid */
struct job_t
{                          /* The job struct */
    pid_t pid;             /* job PID */
    int jid;               /* job ID [1, 2, ...] */
...
struct job_t *getjobdenotion(struct job_t *job_list, const char *str);
...
void eval(char *cmdline)
{
    int bg = -1;          /* should the job run in bg or fg? */
    int fd1 = 0, fd2 = 1; /* stdin, stdout */
    int state = -1;       /* Job state */
    struct cmdline_tokens tok;
    pid_t pid; /* Process id */
    sigset_t mask_all, mask_one, prev_one, mask_two, mask_three;
    Sigfillset(&mask_all);
    Sigemptyset(&mask_one);
    Sigaddset(&mask_one, SIGCHLD);
    Sigemptyset(&prev_one);
    Sigemptyset(&mask_two);
    Sigaddset(&mask_two, SIGHUP); /* nohup */
    Sigemptyset(&mask_three);
    Sigaddset(&mask_three, SIGCHLD);
    Sigaddset(&mask_three, SIGTSTP); /* Ctrl-Z */
    Sigaddset(&mask_three, SIGINT);  /* Ctrl-C */
    /* Parse command line */
    ...
    if (tok.argv[0] == NULL) /* ignore empty lines */
        return;
    switch (tok.builtins) /* Builtin command */
    {
    case BUILTIN_QUIT:
        exit(0);
        break;
    case BUILTIN_NONE:
        break;
    case BUILTIN_NOHUP:
        Sigprocmask(SIG_BLOCK, &mask_two, NULL); /* Block SIGHUP */
        Sigaddset(&mask_one, SIGHUP);
        Sigaddset(&mask_three, SIGHUP);
        for (int i = 0; i < tok.argc - 1; i++) /* Delete "nohup" */
    ...
    case BUILTIN_JOBS:
        if (tok.outfile)
        {
            fd2 = Open(tok.outfile, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
        }
    ...
    case BUILTIN_BG:
    {
        struct job_t *job = getjobdenotion(job_list, tok.argv[1]);
        if (!job)
            return;
        /* Restart job by sending it a SIGCONT signal, and then runs it in the background */
        if (job->state == ST)
        {
            job->state = BG;
    ...
    case BUILTIN_FG:
    {
        ...
        if (!job)
            return;
        /* Restart job by sending it a SIGCONT signal, and then runs it in the foreground */
        if (job->state == ST)
        {
            job->state = FG;
            Kill(-job->pid, SIGCONT);
            flag = 0;
            while (!flag) /* Wait for SIGCHLD to be received */
            {
                Sigsuspend(&prev_one);
            }
        }
    ...
    Sigprocmask(SIG_BLOCK, &mask_three, &prev_one); /* Block SIGCHLD, SIGINT, SIGTSTP (and SIGHUP if nohup) */
    ...
    if ((pid = Fork()) == 0)
    {
        ...
        if (tok.infile)
        {
            ...
            Dup2(fd1, 0); /* Input redirection */
        }
        if (tok.outfile)
        {
            fd2 = Open(tok.outfile, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
            Dup2(fd2, 1); /* Output redirection */
        }
    ...
    Sigprocmask(SIG_BLOCK, &mask_all, NULL);   /* Parent process runs */
    ...
    if (!bg)
    {
        flag = 0;
        while (!flag)
            Sigsuspend(&prev_one); /* Wait for SIGCHLD to be received */
    }
    else
    {
...
void sigchld_handler(int sig)
{
    ...
    Sigfillset(&mask_all);
    Sigemptyset(&prev_one);
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) /* Return any stopped or terminated chldp */
    {
        Sigprocmask(SIG_BLOCK, &mask_all, &prev_one);
        struct job_t *job = getjobpid(job_list, pid);
        if (pid == fgpid(job_list)) /* Foreground process */
            flag = 1;
        if (WIFSTOPPED(status)) /* Stopped Process */
        {
            job->state = ST;
...
/* getjobdenotion  - Find a job (by cmd denotion) on the job list */
struct job_t *getjobdenotion(struct job_t *job_list, const char *str)
{
    ...
    Sigprocmask(SIG_BLOCK, &mask_all, &prev_one);
    int id = -1;
    if (str[0] == '%') /* The job argument is a PID */
    {
        id = atol(str + 1);
        if (id < 0) /* Process group */
            id = -id;
    ...
    else /* The job argument is a JID */
    {
        id = atol(str);
        if (id < 0) /* Process group */
            id = -id;
